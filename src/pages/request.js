import React, { useState, useEffect, useContext, useRef, Switch } from "react";
import {
  Box,
  Grid,
  Typography,
...
export const RequestPage = ({ history }) => {
  ...
  const [windowHeight, setWindowHeigh] = useState(window.innerHeight);
    ...
    axios
      ...
      .then((response) => {
        let responseData = response.data.filter(
          (i) => i.repave_status === "PAVECompleted"
        );
...
const repaveColumns = [
  ...
  },
  {
    ...
    //moment.utc(rowData.promoted_time)
  },
  {
    title: "DATA CENTER",
